...
myAction tempname temph =
    do -- start by display a greeting on the terminal
       putStrLn "Welcome to tempfile.hs"
       putStrLn $ "I have a temporary filr at " ++ tempname
       ...
       putStrLn $ "Which could be expressed as this haskell litera:"
       print c
...
{- This function takes two parameters: a filename pattern and another
   ...
   Ths temporary file is create with openTempFile. The directory is the one
   ...
   exception, the Handle is closed and file is deleted. -}
withTempFile :: String -> (FilePath -> Handle -> IO a) -> IO a
withTempFile pattern func =
    do -- The library ref says that getTemporaryDirectory may raise on
       -- exception on systems that have no notion of a temporary directory.
       -- So, we run getTemporaryDirectory under catchIOError. catchIOError takes
       -- first raised an execption. If getTemporaryDirectory raised an
       ...
       -- whether the first action raised an execption. This way, we ensure
